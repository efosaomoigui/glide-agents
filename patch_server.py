...
new_carousel = """              } else if (post.visual_content.template_type === 'carousel') {
                // ADAPTATION: Handle both strict {slides} format and AI's simplified array format
...
new_status = "status: 'draft', // Forced to draft for dashboard visibility"

if old_carousel in content:
    content = content.replace(old_carousel, new_carousel)
    print('✅ Carousel adaptation logic applied.')
else:
    print('❌ Carousel pattern not found. Check indentation or characters.')

if old_status in content:
    content = content.replace(old_status, new_status)
    print('✅ Forced draft status applied.')
else:
